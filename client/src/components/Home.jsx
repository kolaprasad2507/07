...
const Home = () => {
  ...
  // Mobile-friendly / enhanced cake slices (grid)
  const initialEnhanced = Array(8)
    .fill()
    .map((_, i) => ({ id: i + 1, cut: false }));
  const [enhancedCakeSlices, setEnhancedCakeSlices] = useState(initialEnhanced);
  ...
  // Candle positions based on your uploaded image
...
Your Nanna 💖`;
  ...
  const triggerCelebration = () => {
    ...
    // Extended delay before navigation (3 seconds total)
    setTimeout(() => {
      setCelebrationActive(false);
    }, 1500);
    
    // Navigate to gallery after 3 seconds (extended from 1.5s)
    setTimeout(() => {
      setShowCake(false);
  ...
  // Auto-scroll gallery
  useEffect(() => {
    if (showSlideshow && galleryTrackRef.current) {
      const scrollGallery = () => {
      ...
      const interval = setInterval(() => {
        if (galleryTrackRef.current) {
          galleryTrackRef.current.scrollBy({ left: 1, behavior: 'smooth' });
          scrollGallery();
        }
  ...
  const scrollGallery = (direction) => {
    if (galleryTrackRef.current) {
      const scrollAmount = galleryTrackRef.current.clientWidth;
  ...
  const handleSliceClick = (sliceId, event) => {
    ...
    if (isFirstSlice) {
      setTimeout(() => {
        triggerCelebration();
      }, 500); // Small delay for slice animation
    }
  };

  // Blow candles with 3D effect
  ...
  const handleEnhancedSliceClick = (id, e) => {
    ...
    const isFirstSlice = enhancedCakeSlices.filter(s => s.cut).length === 0;

    setEnhancedCakeSlices((prev) => prev.map((s) => (s.id === id ? { ...s, cut: true } : s)));
  ...
  const resetAll = () => {
    ...
    setEnhancedCakeSlices(initialEnhanced);
  };
    ...
    <div className="birthday-container" onClick={handleParticleClick}>
      ...
      {showCake && !showSlideshow && (
        <div className="cake-scene" role="region" aria-label="Cake scene">
          ...
          {/* Enhanced grid shown on small screens via CSS */}
          <div className="enhanced-cake-section" aria-hidden="false">
            <div className="cake-grid">
              {enhancedCakeSlices.map((slice) => (
                <button
                  key={slice.id}
                  type="button"
                  className={`enhanced-slice ${slice.cut ? 'cut' : ''} ${candlesLit ? 'disabled' : ''}`}
                  onClick={(e) => handleEnhancedSliceClick(slice.id, e)}
                  disabled={slice.cut || candlesLit}
                  aria-pressed={slice.cut}
                >
                  {!slice.cut && <span className="slice-emoji" aria-hidden="true">🍰</span>}
                  {slice.cut && <span className="slice-emoji cut" aria-hidden="true">✅</span>}
                </button>
              ))}
            </div>

            {candlesLit && (
              <div className="enhanced-candles" aria-hidden="true">
                {[1, 2, 3].map((candle) => (
                  <div key={candle} className="enhanced-candle">
                    <div className="enhanced-flame" />
                    <div className="enhanced-candle-stick" />
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Round cake for larger screens */}
          <div className="round-cake-container" aria-hidden={false}>
            {/* Candles positioned directly on cake (SVG coordinates) */}
            {candlesLit && (
            ...
            <svg width="500" height="300" viewBox="0 0 500 300" className="round-cake-svg" role="img" aria-label="Birthday cake">
              <defs>
          ...
          <div className="cake-instructions">
            {candlesLit ? (
              <button type="button" className="blow-candles-btn" onClick={handleBlowCandles}>
                🎂 Blow the Candles! 🕯️
              </button>
            ) : (
